...
class Market:
    ...
    # Market order
        # Sell/buy at the best price available at the market
    # Limit order
        # Sell/buy at the given price or better
    # Stop order
        # Sell/buy at the best price available at the market when the given price is surpassed 
    ...
    def _setup_books(self):
        ...
        make_array = lambda dtype: np.empty(shape=(0,), dtype=dtype)
    ...
    def clear(self):
        # 1. fulfill market and stop orders as they should be executed "immediately"
        # 2. fulfill limit orders
    ...
    def _settle_market_orders(self):
        ...
        # TODO:
        #   If no limit orders, match with market order
        #   using last price
    ...
    @property
    def _has_market_order_bid(self):
        market_bids = self.order_book["market"]["bid"]
        return market_bids.size > 0

    @property
    def _has_market_order_ask(self):
        masket_asks = self.order_book["market"]["ask"]
        return masket_asks.size > 0
...
# Limit order
    def _settle_limit_orders(self):
        ...
        while has_limit_orders(state=self) and self.highest_bid >= self.lowest_ask:
...
# Properties, Bid-Ask
    ...
    @property
    def bid(self):
        "Max bid and quantity"
        bids = self.order_book["limit"]["bid"]
        quantity = bids[bids["price"] == self.highest_bid]["quantity"].sum()
        return self.highest_bid, quantity

    @property
    def ask(self):
        "Min ask and quantity"
        asks = self.order_book["limit"]["ask"]
        quantity = asks[asks["price"] == self.lowest_ask]["quantity"].sum()
        return self.lowest_ask, quantity
...
# Analytical
    ...
    def plot_orders(self, fig=None):
        ...
        sns.distplot(asks["price"], n_bins, rug=True, hist_kws=ask_kwds, kde=False, norm_hist=False, ax=ax_hist, color="r")
        sns.distplot(bids["price"], n_bins, rug=True, hist_kws=bid_kwds, kde=False, norm_hist=False, ax=ax_hist, color="g")
        ax_hist.axvline(self.last_price, 0, 0.25, color="k", linestyle="--")
